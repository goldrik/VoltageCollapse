...
dt = 0.001;       % timestep of calculation
rc_0 = 5;         % Resistance under control (here constant)
rl = 30;          % Load Resistance
e = 9.75;         % Generator voltage output
P_c = 0.0;        % Power demaded at control resistance
...
for P0 = (1:1:P0_last*10);      % loop through each P0 , 1-->P0_last
    ...
    for i = 1:(length(k)-1)     % loop variable i 
        alpha(i+1) = -1*del_p_c(i)/sign(del_rc(i));
...
%     subplot(4,1,1), plot(k, rd_P0{j}, 'color', c);
    subplot(5,1,1), plot(k, rd_P0{j}, 'color', c);
    hold on;
    title('Rd as fx of k');
    xlabel('k');
    ylabel('Rd (Ohms)');
...
%     subplot(4,1,2), plot(k, v_P0{j}, 'color', c);
    subplot(5,1,2), plot(k, v_P0{j}, 'color', c);
    hold on;
    title('V as fx of k');
    xlabel('k');
    ylabel('V (Volts)');
...
%     subplot(4,1,3), plot(k, collapse_P0{j}, 'color', c);
    subplot(5,1,3), plot(k, collapse_P0{j}, 'color', c);
    hold on;
...
%     subplot(4,1,4), plot(k, p_P0{j}, 'color', c);
    subplot(5,1,4), plot(k, p_P0{j}, 'color', c);
    hold on;
    ...
    % rc block
    subplot(5,1,5), plot(k, a_P0{j}, 'color', c);
    hold on;
    title('Rc as fx of k');
    xlabel('k');
    ylabel('Rc (Ohms)');
